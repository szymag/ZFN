...
class EigenMatrix1D:
    def __init__(self, input_parameters, material_A, material_B):
        ...
        self.matrix = np.zeros((2 * self.vectors_count, 2 * self.vectors_count), dtype=complex)
    ...
    def kroneker_delta(self):
        for i in range(self.vectors_count, 2 * self.vectors_count):
            self.matrix[i - self.vectors_count][i] += 1.
            self.matrix[i][i - self.vectors_count] -= 1.

    def exchange_field(self, vec_1, vec_2, bloch_vec):
    ...
    def matrix_angle_dependence(self, bloch_vec):
        ...
        for i in range(indeks, 2 * indeks):
            ...
            ex = self.exchange_field(w1, w2, bloch_vec)
            dyn_in_plane = self.dynamic_demagnetizing_field_in_plane(w1, w2, bloch_vec)
            dyn_out_plane = self.dynamic_demagnetizing_field_out_of_plane(w1, w2, bloch_vec)
            static = self.static_demagnetizing_field(w1, w2)
            self.matrix[i][np.arange(indeks)] += -ex - dyn_in_plane * np.sin\
                (radians(self.parameters.angle())) ** 2 + static * np.cos(radians(self.parameters.angle())) ** 2  # yx
            self.matrix[i - indeks][np.arange(indeks, 2 * indeks)] += \
